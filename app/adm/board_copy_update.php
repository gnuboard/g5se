...
// 게시판 정보 — named placeholder 로 INSERT (컬럼명 = :placeholder 이름 1:1 매칭)
sql_pdo_query(" insert into {$g5['board_table']} set
    bo_table = :bo_table, gr_id = :gr_id, bo_subject = :bo_subject,
    bo_device = :bo_device, bo_admin = :bo_admin,
    bo_list_level = :bo_list_level, bo_read_level = :bo_read_level,
    bo_write_level = :bo_write_level, bo_reply_level = :bo_reply_level,
    ...
    bo_use_rss_view = :bo_use_rss_view, bo_use_good = :bo_use_good,
    bo_use_nogood = :bo_use_nogood, bo_use_name = :bo_use_name,
    bo_use_signature = :bo_use_signature, bo_use_ip_view = :bo_use_ip_view,
    bo_use_list_view = :bo_use_list_view, bo_use_list_content = :bo_use_list_content,
    bo_use_list_file = :bo_use_list_file, bo_table_width = :bo_table_width,
    bo_subject_len = :bo_subject_len, bo_mobile_subject_len = :bo_mobile_subject_len,
    ...
    bo_include_head = :bo_include_head, bo_include_tail = :bo_include_tail,
    ...
    bo_gallery_cols = :bo_gallery_cols, bo_gallery_width = :bo_gallery_width,
    ...
    bo_upload_size = :bo_upload_size, bo_reply_order = :bo_reply_order,
    bo_use_search = :bo_use_search, bo_order = :bo_order,
    bo_notice = :bo_notice, bo_upload_count = :bo_upload_count,
...
[
    ...
    ':bo_mobile_content_head'  => $board['bo_mobile_content_head'],
    ':bo_mobile_content_tail'  => $board['bo_mobile_content_tail'],
...
if ($copy_case == 'schema_data_both') {
    ...
    // 글복사 — write_prefix + bo_table 은 화이트리스트 식별자 (admin 검증)
    sql_pdo_query(" insert into {$g5['write_prefix']}$target_table select * from {$g5['write_prefix']}$bo_table ", [], false);

    // 게시글수 저장
    $row = sql_pdo_fetch(" select bo_count_write, bo_count_comment from {$g5['board_table']} where bo_table = :bo_table ",
                         [':bo_table' => $bo_table]);
    sql_pdo_query(" update {$g5['board_table']} set bo_count_write = :cw, bo_count_comment = :cc where bo_table = :target ",
                  [':cw' => $row['bo_count_write'], ':cc' => $row['bo_count_comment'], ':target' => $target_table], false);

    // 4.00.01 — 파일 메타 복사 대상 조회
    $result = sql_pdo_query(" select * from {$g5['board_file_table']} where bo_table = :bo_table ",
                            [':bo_table' => $bo_table], false);
