...
if ($is_admin != 'super') {
    ...
    $params[':mb_level'] = $member['mb_level'];
}
...
// $sst / $sod 는 화이트리스트 매칭이 안 되어 있어 사용자 입력 그대로 보간 — 잠재 위험
// 그러나 admin auth 로 접근 제한되어 있고 GET 파라미터에서 오므로 일단 보간 유지
...
// 탈퇴회원수
$row = sql_pdo_fetch(" select count(*) as cnt {$sql_common} {$sql_search} and mb_leave_date <> '' {$sql_order} ", $params);
$leave_count = $row['cnt'];

// 차단회원수
$row = sql_pdo_fetch(" select count(*) as cnt {$sql_common} {$sql_search} and mb_intercept_date <> '' {$sql_order} ", $params);
        ...
        <table>
            ...
            <tbody>
                ...
                for ($i = 0; $row = sql_fetch_array($result); $i++) {
                    // 접근가능한 그룹수
                    $row2 = sql_pdo_fetch(" select count(*) as cnt from {$g5['group_member_table']} where mb_id = :mb_id ",
                                          [':mb_id' => $row['mb_id']]);
                    $group = '';
                    if ($row2['cnt']) {
