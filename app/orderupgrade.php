...
if(!sql_pdo_query(" select it_sc_type from {$g5['g5_shop_cart_table']} limit 1 ", false)) {
...
if(!sql_pdo_query(" select ct_select_time from {$g5['g5_shop_cart_table']} limit 1 ", false)) {
    sql_pdo_query(" ALTER TABLE `{$g5['g5_shop_cart_table']}`
                    ADD `ct_select_time` datetime NULL DEFAULT NULL AFTER `ct_select` ", true);
}

// 모바일 이니시스 계좌이체 결과 전달을 위한 테이블 추가
if(!sql_pdo_query(" DESCRIBE {$g5['g5_shop_inicis_log_table']} ", false)) {
    sql_pdo_query(" CREATE TABLE IF NOT EXISTS `{$g5['g5_shop_inicis_log_table']}` (
...
// 결제정보 임시저장 테이블 추가
if(isset($g5['g5_shop_order_data_table']) && !sql_pdo_query(" DESCRIBE {$g5['g5_shop_order_data_table']} ", false)) {
    sql_pdo_query(" CREATE TABLE IF NOT EXISTS `{$g5['g5_shop_order_data_table']}` (
...
if(!sql_pdo_query(" select P_AUTH_NO from {$g5['g5_shop_inicis_log_table']} limit 1 ", false)) {
    sql_pdo_query(" ALTER TABLE `{$g5['g5_shop_inicis_log_table']}`
                    ADD `P_AUTH_NO` varchar(255) NOT NULL DEFAULT '' AFTER `P_FN_NM` ", true);
}

// 모바일 이니시스 noti 받는 테이블 필드 추가
if(!sql_pdo_query(" select post_data from {$g5['g5_shop_inicis_log_table']} limit 1 ", false)) {
    sql_pdo_query(" ALTER TABLE `{$g5['g5_shop_inicis_log_table']}`
...
// 테스트 주문필드 추가
if(!sql_pdo_query(" select od_test from {$g5['g5_shop_order_table']} limit 1 ", false)) {
...
if(!sql_pdo_query(" select cart_id from {$g5['g5_shop_order_data_table']} limit 1 ", false)) {
    sql_pdo_query(" ALTER TABLE `{$g5['g5_shop_order_data_table']}`
...
if(isset($g5['g5_shop_coupon_zone_table'])) {
    if(!sql_pdo_query(" DESCRIBE {$g5['g5_shop_coupon_zone_table']} ", false)) {
        sql_pdo_query(" CREATE TABLE IF NOT EXISTS `{$g5['g5_shop_coupon_zone_table']}` (
...
// 쿠폰테이블에 cz_id 필드 추가
if(!sql_pdo_query(" select cz_id from {$g5['g5_shop_coupon_table']} limit 1 ", false)) {
    sql_pdo_query(" ALTER TABLE `{$g5['g5_shop_coupon_table']}`
