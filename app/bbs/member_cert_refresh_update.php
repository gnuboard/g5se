...
if ($config['cf_cert_use'] && $cert_type && $md5_cert_no) {
    ...
    if ($hash_ok_ipin || $hash_ok_others) {
        $sql_certify = " mb_hp       = :cert_mb_hp,
                          mb_certify  = :cert_mb_certify,
                          mb_adult    = :cert_mb_adult,
                          mb_birth    = :cert_mb_birth,
                          mb_sex      = :cert_mb_sex,
                          mb_dupinfo  = :cert_mb_dupinfo,
                          mb_name     = :cert_mb_name ";
        $cert_params = [
            ':cert_mb_hp'      => $mb_hp,
            ':cert_mb_certify' => $cert_type,
            ':cert_mb_adult'   => get_session('ss_cert_adult'),
            ':cert_mb_birth'   => get_session('ss_cert_birth'),
            ':cert_mb_sex'     => get_session('ss_cert_sex'),
            ':cert_mb_dupinfo' => get_session('ss_cert_dupinfo'),
            ':cert_mb_name'    => $mb_name,
        ];
    } else {
...
$result = false;
if ($sql_certify) {
    $cert_params[':mb_id'] = $mb_id;
    $result = sql_pdo_query("update {$g5['member_table']} set {$sql_certify} where mb_id = :mb_id",
                            $cert_params, false);
}
