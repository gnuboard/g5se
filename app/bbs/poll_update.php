...
// $gb_poll 은 위에서 [^0-9] 제거 — 컬럼명 보간 안전
if ($is_member) {
    sql_pdo_query(" update {$g5['poll_table']} set
                        po_cnt{$gb_poll} = po_cnt{$gb_poll} + 1,
                        mb_ids = concat(ifnull(mb_ids, ''), :mb_id_csv)
                    where po_id = :po_id
                      and find_in_set(:mb_id_match, ifnull(mb_ids, '')) = 0 ",
                  [':mb_id_csv' => $member['mb_id'].',', ':mb_id_match' => $member['mb_id'], ':po_id' => $po_id]);
} else {
    sql_pdo_query(" update {$g5['poll_table']} set
                        po_cnt{$gb_poll} = po_cnt{$gb_poll} + 1,
                        po_ips = concat(ifnull(po_ips, ''), :ip_csv)
                    where po_id = :po_id
                      and find_in_set(:ip_match, ifnull(po_ips, '')) = 0 ",
                  [':ip_csv' => $_SERVER['REMOTE_ADDR'].',', ':ip_match' => $_SERVER['REMOTE_ADDR'], ':po_id' => $po_id]);
}
