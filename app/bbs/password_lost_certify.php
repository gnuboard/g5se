...
$mb = sql_pdo_fetch(" select mb_id, mb_lost_certify from {$g5['member_table']} where mb_no = :mb_no ",
                    [':mb_no' => $mb_no]);
if (strlen($mb['mb_lost_certify']) < 33)
    die("Error");

// 인증 링크는 한번만 처리가 되게 한다.
sql_pdo_query(" update {$g5['member_table']} set mb_lost_certify = '' where mb_no = :mb_no ",
              [':mb_no' => $mb_no]);
...
if ($mb_nonce === substr($mb['mb_lost_certify'], 0, 32)) {
    ...
    sql_pdo_query(" update {$g5['member_table']} set mb_password = :mb_password where mb_no = :mb_no ",
                  [':mb_password' => $new_password_hash, ':mb_no' => $mb_no]);
