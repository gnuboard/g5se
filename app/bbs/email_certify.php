...
$row = sql_pdo_fetch(" select mb_id, mb_email_certify2, mb_leave_date, mb_intercept_date from {$g5['member_table']} where mb_id = :mb_id ",
                     [':mb_id' => $mb_id]);
if (!$row['mb_id'])
...
sql_pdo_query(" update {$g5['member_table']} set mb_email_certify2 = '' where mb_id = :mb_id ",
              [':mb_id' => $mb_id]);
...
if ($mb_md5)
{
    ...
    {
        sql_pdo_query(" update {$g5['member_table']} set mb_email_certify = :certify where mb_id = :mb_id ",
                      [':certify' => G5_TIME_YMDHIS, ':mb_id' => $mb_id]);
