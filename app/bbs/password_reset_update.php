...
$mb_check = sql_pdo_fetch("select * from {$g5['member_table']} where mb_id = ? AND mb_dupinfo = ?", [$mb_id, $mb_dupinfo]);
...
sql_pdo_query("update {$g5['member_table']} set mb_password = ? where mb_id = ? AND mb_dupinfo = ?",
              [get_encrypt_string($mb_password), $mb_id, $mb_dupinfo]);
