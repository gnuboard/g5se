...
for($i=0;$i<$count_chk_bn_id;$i++)
{
    ...
    if ($board['bo_table'] != $bo_table)
        $board = sql_pdo_fetch(" select bo_subject, bo_write_point, bo_comment_point, bo_notice from {$g5['board_table']} where bo_table = :bo_table ",
                               [':bo_table' => $bo_table]);
    ...
    if ($write['wr_is_comment']==0)
    {
        ...
        $result = sql_pdo_query(" select wr_id, mb_id, wr_is_comment from $write_table where wr_parent = :wr_parent order by wr_id ",
        ...
        if ($pressed == '선택내용삭제') {
            // 게시글 내용만 삭제
            sql_pdo_query(" update $write_table set
                                wr_subject = :wr_subject,
                                wr_content = '',
                                wr_name    = '본인요청삭제☆'
                            where wr_id = :wr_id ",
                          [':wr_subject' => G5_TIME_YMDHIS.' - 본인 요청으로 인한 삭제 (냉무) ☆', ':wr_id' => $write['wr_id']]);
        } else {
            // 게시글 삭제
    ...
    else // 코멘트 삭제
    {
        ...
        // 코멘트 삭제
        sql_pdo_query(" delete from $write_table where wr_id = :wr_id ", [':wr_id' => $comment_id]);

        // 코멘트가 삭제되므로 해당 게시물에 대한 최근 시간을 다시 얻는다.
        $row = sql_pdo_fetch(" select max(wr_datetime) as wr_last from $write_table where wr_parent = :wr_parent ",
                             [':wr_parent' => $write['wr_parent']]);

        // 원글의 코멘트 숫자를 감소
        sql_pdo_query(" update $write_table set wr_comment = wr_comment - 1, wr_last = :wr_last where wr_id = :wr_id ",
                      [':wr_last' => $row['wr_last'], ':wr_id' => $write['wr_parent']]);
        ...
        sql_pdo_query(" delete from {$g5['board_new_table']} where bo_table = :bo_table and wr_id = :wr_id ",
                      [':bo_table' => $bo_table, ':wr_id' => $comment_id]);
    }
}
