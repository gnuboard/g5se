...
$write = sql_pdo_fetch(" select * from {$write_table} where wr_id = :wr_id ", [':wr_id' => $comment_id]);
...
$row = sql_pdo_fetch(" select count(*) as cnt from {$write_table}
            ...
            and wr_is_comment = 1 ",
            [':reply' => $comment_reply.'%', ':wr_id' => $comment_id, ':wr_parent' => $write['wr_parent'], ':wr_comment' => $write['wr_comment']]);
if ($row['cnt'] && !$is_admin)
