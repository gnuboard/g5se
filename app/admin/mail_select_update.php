<?php
/*
 * /admin/mail_select_update — 메일 일괄 발송 (스트리밍 진행 출력).
 */
$sub_menu = "200300";
...
<?php
echo '</div></main>'; admin_layout_end();
...
// 메일내용 가져오기
$ma = sql_pdo_fetch(" select ma_subject, ma_content from {$g5['mail_table']} where ma_id = :ma_id ", [':ma_id' => $ma_id]);
...
for ($i = 0; $i < count($member_list); $i++) {
    ...
    if ($sw == true) {
        ...
        $content = $content . "<hr size=0><p><span style='font-size:9pt; font-family:굴림'>▶ 더 이상 정보 수신을 원치 않으시면 [<a href='" . G5_URL . "/email_stop?mb_id=" . rawurlencode($mb_id) . "&amp;mb_md5={$mb_md5}' target='_blank'>수신거부</a>] 해 주십시오.</span></p>";
