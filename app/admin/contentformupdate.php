<?php
/*
 * /admin/contentformupdate — 내용(컨텐츠) 저장/삭제 POST 핸들러.
 */
$sub_menu = '300600';
...
if ($w == "" || $w == "u") {
    ...
    $co_row = sql_pdo_fetch(" select * from {$g5['content_table']} where co_id = :co_id ", [':co_id' => $co_id]);
}
...
$co_skin = isset($_POST['co_skin']) ? clean_xss_tags(stripslashes($_POST['co_skin']), 1, 1) : '';
$co_mobile_skin = isset($_POST['co_mobile_skin']) ? clean_xss_tags(stripslashes($_POST['co_mobile_skin']), 1, 1) : '';
...
if ($w == "") {
    ...
    sql_pdo_query(" insert {$g5['content_table']} set co_id = :co_id, $sql_common ",
                  array_merge([':co_id' => $co_id], $common_params));
    run_event('admin_content_created', $co_id);
} elseif ($w == "u") {
    sql_pdo_query(" update {$g5['content_table']} set $sql_common where co_id = :co_id ",
                  array_merge($common_params, [':co_id' => $co_id]));
    run_event('admin_content_updated', $co_id);
} elseif ($w == "d") {
    @unlink(G5_DATA_PATH . "/content/{$co_id}_h");
    @unlink(G5_DATA_PATH . "/content/{$co_id}_t");

    sql_pdo_query(" delete from {$g5['content_table']} where co_id = :co_id ", [':co_id' => $co_id]);
...
if ($w == "" || $w == "u") {
    ...
    if ($error_msg) {
        alert($error_msg, G5_ADMIN_URL."/contentform?w=u&co_id=$co_id");
    } else {
        header('Location: '.G5_ADMIN_URL."/contentform?w=u&co_id=$co_id", true, 302);
        exit;
    }
} else {
    header('Location: '.G5_ADMIN_URL.'/contentlist', true, 302);
    exit;
}
