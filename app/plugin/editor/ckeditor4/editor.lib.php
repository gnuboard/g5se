...
function editor_html($id, $content, $is_dhtml_editor=true)
{
    ...
    if ($is_dhtml_editor && $js) {
        ...
        $version_date = (defined('G5_JS_VER') && G5_JS_VER) ? G5_JS_VER : preg_replace('/[^0-9]/i', '', G5_TIME_YMD);
        // ckeditor4 자산은 모두 /app/plugin/editor/ckeditor4 한 곳에서 로드 (원본은
    ...
    if($is_dhtml_editor) {
        ...
        // textLength = text.replace(/<([^>]+)>/gi, '').replace(/\s/g, '').length;
        ...
        $html .= "
            ...
            CKEDITOR.on( 'instanceLoaded', function (e) {
                ...
                if (editor_id === 'wr_content') {   // 게시판에서 글쓰기 할때만 사용
                    ...
                    editor.on('change', function() {
                        typewatch(function(){
                            ...
                            var tmp_str_content = div.textContent || div.innerText || '',
                                textLength = tmp_str_content.replace(/\s/g, '').length;

                            // 엘리먼트의 ID
                            var elementId = editor_id + '_textLength';

                            // 엘리먼트 존재 여부 확인
                            textLength = '(' + textLength + ')글자'
                            if ($('#' + elementId).length === 0) {
                                // 엘리먼트 생성
                                $('.write_preview_group_box .btn_save_temp').after($('<p>').attr('id', elementId).addClass('btn_cke_sc').css('margin-left', '5px').text(textLength));
                            } else {
                                if (textLength) {
                                    $('#' + elementId).text(textLength);
                                } else {
                                    $('#' + elementId).remove();
                                }
                            }
...
function chk_editor_js($id, $is_dhtml_editor=true)
{
    ...
    if ($is_dhtml_editor) {
        ...
        CKEDITOR.instances.{$id}.setData(temp_data);".PHP_EOL;
        
        if (isset($board['bo_write_min']) && $board['bo_write_min']){   // 게시판 설정에서 최소 글자수 값이 설정되어 있으면
            $print_js .= "var div = document.createElement('div');".PHP_EOL;
            $print_js .= "div.innerHTML = {$id}_editor_data;".PHP_EOL;
            $print_js .= "var tmp_str_content = div.textContent || div.innerText || '';".PHP_EOL;
            // $print_js .= "var tmp_str_length = ({$id}_editor_data).replace(/(<([^>]+)>)/gi, '').length;".PHP_EOL;
            // $print_js .= "console.log(tmp_str_content, tmp_str_content.length, tmp_str_content.replace(/\s/g, '').length);".PHP_EOL;
            // $print_js .= "return false;".PHP_EOL;
            
            //$print_js .= "var tmp_str_content = {$id}_editor_data.replace(/<[^>]*>/g, '');".PHP_EOL;
            $print_js .= "var bo_write_min = {$board['bo_write_min']} || 0;".PHP_EOL;
            $print_js .= "var now_write_length = tmp_str_content.replace(/\s/g, '').length;".PHP_EOL;
            $print_js .= "if (bo_write_min && now_write_length < bo_write_min) { alert(\"(\" + now_write_length + \")글자 / ".$board['bo_write_min']."자 이상 입력하셔야 합니다.\"); return false; }\n";
        }
    } else {
