...
<div id="sod_frm" class="sod_frm_pc">
    ...
    <div class="sod_left">
        ...
        <section id="sod_frm_orderer">
            <h2>주문하시는 분</h2>
            <?php if ($is_member) { ?>
            <div class="orderer-member-save">
                <input type="checkbox" name="save_orderer_to_member" value="1" id="save_orderer_to_member">
                <label for="save_orderer_to_member">회원정보에 저장</label>
            </div>
            <?php } ?>
            ...
            <div class="tbl_frm01 tbl_wrap">
                ...
                <tr>
                    ...
                    <td>
                        <input type="hidden" name="od_tel" value="<?php echo get_text($member['mb_hp'] ?: $member['mb_tel']); ?>" id="od_tel">
                        <input type="tel" name="od_hp" value="<?php echo get_text($member['mb_hp'] ?: $member['mb_tel']); ?>" id="od_hp" required class="frm_input required js-phone-number" maxlength="14" inputmode="numeric" autocomplete="tel">
                    </td>
                </tr>
                <tr>
                    <th scope="row">주소</th>
            ...
            <div class="tbl_frm01 tbl_wrap">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="tel" name="od_b_hp" id="od_b_hp" required class="frm_input required js-phone-number" maxlength="14" inputmode="numeric" autocomplete="tel">
                    </td>
                </tr>
                <tr>
                    <th scope="row">주소</th>
...
function formatKoreanPhone(value) {
    var number = String(value || "").replace(/\D/g, "").slice(0, 12);

    // 15xx/16xx/18xx 대표번호
    if (/^(15|16|18)/.test(number)) {
        return number.length > 4 ? number.slice(0, 4) + "-" + number.slice(4, 8) : number;
    }

    // 050x 안심번호: 0504-1234-5678
    if (/^050[2-8]/.test(number)) {
    ...
    // 서울 지역번호: 02-123-4567 / 02-1234-5678
    ...
    // 휴대폰 및 기타 지역번호: 010-1234-5678 / 031-123-4567
...
$(function() {
    ...
    $(document).on("input", ".js-phone-number", function() {
        ...
        if (this.id === "od_hp") $("#od_tel").val(this.value);
        if (this.id === "od_b_hp") $("#od_b_tel").val(this.value);
    });
    ...
    $("input[name=ad_sel_addr]").on("click", function() {
        ...
        if (addr[0] == "same") {
            ...
            f.od_b_hp.value          = formatKoreanPhone(addr[2] || addr[1]);
            f.od_b_tel.value         = f.od_b_hp.value;
...
function forderform_check(f)
{
    ...
    // 화면에는 연락처 하나만 노출하되 기존 PG/주문 DB 호환을 위해 두 필드를 동기화한다.
    f.od_hp.value = formatKoreanPhone(f.od_hp.value);
    f.od_tel.value = f.od_hp.value;
    f.od_b_hp.value = formatKoreanPhone(f.od_b_hp.value);
    f.od_b_tel.value = f.od_b_hp.value;
...
function gumae2baesong() {
    ...
    f.od_b_tel.value  = f.od_hp.value;
