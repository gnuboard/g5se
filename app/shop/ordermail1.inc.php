...
//------------------------------------------------------------------------------
// Loop 배열 자료를 만들고
$result = sql_pdo_query(" select a.it_id, a.it_name, a.ct_qty, a.ct_price, a.ct_point,
                                 b.it_sell_email, b.it_origin
                            from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on ( a.it_id = b.it_id )
                           where a.od_id = :od_id and a.ct_select = '1'
                           group by a.it_id order by a.ct_id asc ",
                       [':od_id' => $od_id]);
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    $sum = sql_pdo_fetch(" select SUM(IF(io_type = 1, (io_price * ct_qty), ((ct_price + io_price) * ct_qty))) as price,
                            ...
                            where it_id = :it_id and od_id = :od_id and ct_select = '1' ",
                        [':it_id' => $row['it_id'], ':od_id' => $od_id]);

    // 옵션정보
    $result2 = sql_pdo_query(" select ct_option, ct_qty, io_price
                                 from {$g5['g5_shop_cart_table']}
                                where it_id = :it_id and od_id = :od_id and ct_select = '1'
