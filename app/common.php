...
if (file_exists($dbconfig_file)) {
    ...
    if(defined('G5_MYSQL_SET_MODE') && G5_MYSQL_SET_MODE) sql_pdo_query("SET SESSION sql_mode = ''");
    if (defined('G5_TIMEZONE')) sql_pdo_query(" set time_zone = :tz ", [':tz' => G5_TIMEZONE]);
...
<main>
    ...
    <div id="g5se-rw-warn" class="card" hidden style="border-color:#dc2626;border-left:4px solid #dc2626;">
        <h2 style="margin:0 0 12px;font-size:1.15em;color:#dc2626;display:flex;align-items:center;gap:8px;">
        ...
        <p>이 사이트는 <code>/css/…</code>, <code>/img/…</code>, <code>/theme/…</code> 같은 <strong>짧은 주소</strong>를 실제 파일 위치로 연결해 주는 설정이 필요합니다. 지금 웹서버에 이 설정이 없어서 CSS·이미지·자바스크립트가 모두 깨집니다.</p>
        <p>설치는 끝까지 진행되지만, <strong>설치 후 화면이 깨져 보입니다.</strong> 웹서버 담당자에게 아래 설정을 요청하세요.</p>
        <details style="margin-top:8px;">
            <summary style="cursor:pointer;color:var(--text-soft);">서버 담당자용 설정 방법</summary>
            <p style="margin:8px 0;color:var(--text-soft);">짧은 주소 <code>/(theme|skin|img|js|css|mobile|plugin)/*</code> 를 <code>/app/$1/*</code> 로 연결해야 합니다.</p>
            <ul style="margin:8px 0;padding-left:20px;color:var(--text-soft);">
                <li><strong>Apache</strong>: <code>mod_rewrite</code> 활성화 + <code>AllowOverride All</code> (이미 <code>.htaccess</code> 에 규칙이 들어 있음)</li>
                <li><strong>nginx</strong>: <code>.htaccess</code> 의 규칙을 nginx <code>rewrite</code> / <code>try_files</code> 로 옮기거나, docroot 에 <code>theme → app/theme</code> 형태의 symlink 추가</li>
            </ul>
        </details>
        <p style="margin-top:12px;font-size:0.9em;color:var(--text-faint);">현재 서버: <code id="g5se-rw-srv">?</code></p>
    </div>
</main>
...
<script>
// 라우팅 자가진단 — /css/default.css 가 닿으면 OK, 404 면 경고 배너 노출.
// .htaccess RewriteRule 누락 또는 nginx 동등 설정 미적용 환경 감지.
(function () {
    var probe = '<?php echo G5_URL; ?>/css/default.css?_g5se_rw_probe=1';
    fetch(probe, { method: 'HEAD', cache: 'no-store' })
        .then(function (r) {
            if (r.ok) return;
            var w = document.getElementById('g5se-rw-warn');
            var s = document.getElementById('g5se-rw-srv');
            if (w) w.hidden = false;
            if (s) s.textContent = <?php echo json_encode($_SERVER['SERVER_SOFTWARE'] ?? '?'); ?>;
        })
        .catch(function () {});
})();
</script>
...
if (isset($_SESSION['ss_mb_id']) && $_SESSION['ss_mb_id']) { // 로그인중이라면
    ...
    if( ($member['mb_intercept_date'] && $member['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) 
        ...
        if (substr($member['mb_today_login'], 0, 10) != G5_TIME_YMD) {
            ...
            sql_pdo_query(" update {$g5['member_table']} set mb_today_login = :now, mb_login_ip = :login_ip where mb_id = :mb_id ",
                          [':now' => G5_TIME_YMDHIS, ':login_ip' => $_SERVER['REMOTE_ADDR'], ':mb_id' => $member['mb_id']]);
        }
    }
} else {
    // 자동로그인 ---------------------------------------
    ...
    if ($tmp_mb_id = get_cookie('ck_mb_id')) {
        ...
        if (strtolower($tmp_mb_id) !== strtolower($config['cf_admin'])) {
            ...
            if ($tmp_key && preg_match('/^[a-f0-9]{64}$/', $tmp_key)) {
                ...
                $row = sql_pdo_fetch(" select al.al_id, al.al_expire,
                          ...
                          where al.mb_id = :mb_id
                            and al.al_token = :al_token
                            and al.al_expire > :now ",
                          [':mb_id' => $tmp_mb_id, ':al_token' => $tmp_key_hash, ':now' => G5_TIME_YMDHIS]);
                if (isset($row['al_id']) && $row['al_id']) {
                    ...
                    if ($row['mb_intercept_date'] == '' &&
                        ...
                        // 마지막 사용 시간 갱신
                        sql_pdo_query(" update {$g5['member_auto_login_table']}
                                       set al_last_used = :now
                                     where al_id = :al_id ",
                                       [':now' => G5_TIME_YMDHIS, ':al_id' => $row['al_id']]);
